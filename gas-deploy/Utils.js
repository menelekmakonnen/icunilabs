...
function sheetToObjects_(sheetName) {
    ...
    // This prevents duplicate trailing headers from overwriting correct values.
    var colLimit = expectedCols ? expectedCols.length : headers.length;
    var results = [];
    for (var i = 1; i < data.length; i++) {
        var obj = { _rowIndex: i + 1 };
        for (var j = 0; j < colLimit && j < headers.length; j++) {
 ...
 * 1. Writes each expected header into its correct column position.
 * 2. Deletes any extra columns beyond the expected count (duplicate cleanup).
 */
function ensureHeaders_(sheet, expectedHeaders) {
    ...
    // Delete any extra columns beyond the expected count (duplicate cleanup)
    var actualLastCol = sheet.getLastColumn();
    if (actualLastCol > expectedHeaders.length) {
        var extraCols = actualLastCol - expectedHeaders.length;
        sheet.deleteColumns(expectedHeaders.length + 1, extraCols);
        Logger.log('ensureHeaders_: Deleted ' + extraCols + ' extra columns from ' + sheet.getName());
        fixed += extraCols;
    }
    if (fixed > 0) {
        Logger.log('ensureHeaders_: Fixed ' + fixed + ' issues in ' + sheet.getName());
