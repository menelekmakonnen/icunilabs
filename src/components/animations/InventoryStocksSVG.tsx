...
export default function InventoryStocksSVG() {
    ...
    return (
        <div ref={ref} className="w-full relative aspect-square sm:aspect-[4/3] rounded-2xl border border-neutral-800/60 bg-neutral-950/80 shadow-2xl overflow-hidden backdrop-blur-xl flex items-center justify-center p-4">
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-3/4 h-3/4 bg-[#ff7a00]/5 blur-[80px] rounded-full pointer-events-none" />
                ...
                <motion.g style={{ y: rowsY }}>
                    {[...Array(6)].map((_, i) => (
                        <motion.g key={i} transform={`translate(40, ${200 + i * 60})`} style={{ opacity: i < 5 ? opacities[i] : 0 }}>
                            ...
                            <circle cx="20" cy="25" r="8" fill={i % 3 === 0 ? "#ff7a00" : "#00bfff"} fillOpacity="0.8" />
                            ...
                            <rect x="410" y="20" width={40 + ((i * 37) % 80)} height="10" rx="5" fill={i % 3 === 0 ? "#ff7a00" : "#00bfff"} />
                            
                            {/* Status Pill */}
                            <rect x="610" y="15" width="80" height="20" rx="10" fill={i % 3 === 0 ? "#ff7a0022" : "#00bfff22"} stroke={i % 3 === 0 ? "#ff7a00" : "#00bfff"} strokeWidth="1" />
                        </motion.g>
                    ))}
                </motion.g>
